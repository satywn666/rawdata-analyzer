...
# Subset for filtered (total_hrs > 10) and full
filtered_data = df_data[df_data['total_hrs'] > 10].copy()

# Add calculations
df_data['actual_speed'] = df_data['miles_slc'] / df_data['total_hrs']
df_data['slip'] = 1 - ((df_data['actual_speed'] * 1852) / (df_data['engine_rpm'] * df_data['prop_pitch'] * 60))
...
general_table = filtered_data[['telegram_date', 'daily_fo_cons', 'fw_consumption', 'fw_production', 'performance_speed', 'actual_speed']]
for r in dataframe_to_rows(general_table, index=False, header=True):
    ws1.append(r)

ws1.append([])
ws1.append(["Date", "Speed Lost by Current", "Speed Lost by Swell", "Speed Lost by Wind"])
for i in range(len(filtered_data)):
    ws1.append([filtered_data['telegram_date'].iloc[i], filtered_data['current_loss'].iloc[i], filtered_data['swell_loss'].iloc[i], filtered_data['wind_loss'].iloc[i]])

bar1 = BarChart()
bar1.title = "Fuel, FW, and Speed Performance"
bar1.add_data(Reference(ws1, min_col=2, min_row=2, max_col=6, max_row=1+len(general_table)), titles_from_data=True)
bar1.set_categories(Reference(ws1, min_col=1, min_row=2, max_row=1+len(general_table)))
ws1.add_chart(bar1, "I2")

start_row = len(general_table) + 5
bar2 = BarChart()
bar2.title = "Speed Loss by Weather"
bar2.add_data(Reference(ws1, min_col=2, min_row=start_row, max_col=4, max_row=start_row+len(filtered_data)-1), titles_from_data=False)
ws1.add_chart(bar2, "I20")

# ---- Sheet 2: Main Engine Data ----
ws2 = wb.create_sheet("Main Engine Data")
me_data = filtered_data[['telegram_date', 'engine_rpm', 'slip', 'me_hsfo', 'me_lsfo', 'come_cons_delta']].copy()
